...
def _generate_symmetrized_order_parameters(
    results: Union[WangLandauResults, MulticanonicalResults]
    ...
    energy_coeff = (
        abs(results.model.J) * (results.model.spin_scale_factor / 2) ** results.model.p
    )
    order_parameter_results = n_op.to_order_parameter_results(energy_coeff)

    # もとあったオブジェクトをコピーしておく
    order_parameter_results.squared_magnetization = (
        results.order_parameters.squared_magnetization.copy()
    )
    order_parameter_results.forth_magnetization = (
        results.order_parameters.forth_magnetization.copy()
    )
